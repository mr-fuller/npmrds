...
add column am_peak_dwdh_2011 numeric;
...
where date_part('hour', measurement_tstamp)  > 5 and date_part('hour', measurement_tstamp)  < 9 and date_part('year', measurement_tstamp) = 2011
group by tmc_code
)
...
where cl.tmc_code = am_peak_dwdh_2011.tmc_code ;--and (date_part('hour', measurement_tstamp)  > 8 and date_part('hour', measurement_tstamp)  < 14);
