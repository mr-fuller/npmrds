...
alter table tttr
add column if not exists tttr_pmp_2016 numeric,
add column if not exists ttt_pmp50pct_2016 numeric,
add column if not exists ttt_pmp95pct_2016 numeric;
...
tmc_code,
--geom,
round(cast(percentile_disc(0.95) within group (order by travel_time_seconds) as numeric),0) as ttt_pmp95pct_2016,
round(cast(percentile_disc(0.5) within group (order by travel_time_seconds) as numeric), 0) as ttt_pmp50pct_2016,
...
update tttr
set tttr_pmp_2016 = apl.tttr,
ttt_pmp50pct_2016 = apl.ttt_pmp50pct_2016,
ttt_pmp95pct_2016 = apl.ttt_pmp95pct_2016
from apl
