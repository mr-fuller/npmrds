...
add column if not exists we_dwdm_2016 numeric,
add column if not exists we_dwdm_pct_2016 numeric;
update passenger_dwdm
set we_dwdm_2016 = null;
...
(
	select
tmc_code,
round(sum(delay_minutes*miles),2) as we_dwdm_2016,
round(sum(delay_minutes*miles)/(14*2*52*60)*100,2) as we_dwdm_pct_2016
...
set we_dwdm_2016 = we_dwdm_2016.we_dwdm_2016,
we_dwdm_pct_2016 = we_dwdm_2016.we_dwdm_pct_2016
from we_dwdm_2016
where cl.tmc_code = we_dwdm_2016.tmc_code ;--and (date_part('hour', measurement_tstamp)  > 8 and date_part('hour', measurement_tstamp)  < 14);
