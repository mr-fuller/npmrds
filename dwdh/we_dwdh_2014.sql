...
add column if not exists we_dwdm_2014 numeric,
add column if not exists we_dwdm_pct_2014 numeric;
update passenger_dwdh
set we_dwdm_2014 = null;
...
(select
tmc_code,
round(sum(delay_minutes*miles),2) as we_dwdm_2014,
round(sum(delay_minutes*miles)/(14*2*52*60)*100,2) as we_dwdm_pct_2014
...
set we_dwdm_2014 = we_dwdm_2014.we_dwdm_2014,
we_dwdm_pct_2014 = we_dwdm_2014.we_dwdm_pct_2014
from we_dwdm_2014
where cl.tmc_code = we_dwdm_2014.tmc_code ;--and (date_part('hour', measurement_tstamp)  > 8 and date_part('hour', measurement_tstamp)  < 14);
