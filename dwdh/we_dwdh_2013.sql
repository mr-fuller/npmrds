...
(select
tmc_code,
round(sum(delay_minutes*miles),2) as we_dwdm_2013,
round(sum(delay_minutes*miles)/(14*2*52*60)*100,2) as we_dwdm_pct_2013
...
from we_dwdm_2013
where cl.tmc_code = we_dwdm_2013.tmc_code ;--and (date_part('hour', measurement_tstamp)  > 8 and date_part('hour', measurement_tstamp)  < 14);
