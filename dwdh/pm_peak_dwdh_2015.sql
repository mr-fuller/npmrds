...
add column if not exists pmp_dwdm_2015 numeric,
add column if not exists pmp_dwdm_pct_2015 numeric;
...
(select
tmc_code,
round(sum(delay_minutes*miles),2) as pmp_dwdm_2015,
round(sum(delay_minutes*miles)/(4*5*52*60)*100,2) as pmp_dwdm_pct_2015
...
set pmp_dwdm_2015 = pmp_dwdm_2015.pmp_dwdm_2015,
pmp_dwdm_pct_2015 = pmp_dwdm_2015.pmp_dwdm_pct_2015
from pmp_dwdm_2015
where cl.tmc_code = pmp_dwdm_2015.tmc_code ;--and (date_part('hour', measurement_tstamp)  > 8 and date_part('hour', measurement_tstamp)  < 14);
