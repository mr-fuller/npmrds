...
add column if not exists we_dwdm_2012 numeric,
add column if not exists we_dwdm_pct_2012 numeric;
update passenger_dwdm
set we_dwdm_2012 = null;
...
(select
tmc_code,
round(sum(delay_minutes*miles),2) as we_dwdm_2012,
round(sum(delay_minutes*miles)/(14*2*52*60)*100,2) as we_dwdm_pct_2012
...
set we_dwdm_2012 = we_dwdm_2012.we_dwdm_2012,
we_dwdm_pct_2012 = we_dwdm_2012.we_dwdm_pct_2012
from we_dwdm_2012
where cl.tmc_code = we_dwdm_2012.tmc_code ;--and (date_part('hour', measurement_tstamp)  > 8 and date_part('hour', measurement_tstamp)  < 14);
