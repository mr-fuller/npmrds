...
(select
tmc_code,
round(sum(delay_minutes*miles),2) as midday_dwdm_2013,
round(sum(delay_minutes*miles)/(6*5*52*60)*100,2) as midday_dwdm_pct_2013
...
where cl.tmc_code = midday_dwdm_2013.tmc_code ;--and (date_part('hour', measurement_tstamp)  > 8 and date_part('hour', measurement_tstamp)  < 14);
