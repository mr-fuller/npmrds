drop table if exists lmw_2021_inrix_truck_seconds_nonull_10min;
create table lmw_2021_inrix_truck_seconds_nonull_10min (
tmc_code varchar(50),
...
\copy lmw_2021_inrix_truck_seconds_nonull_10min from program 'unzip -p /home/fullerm/repos/npmrds/downloads/2021/truck/lmw_2021_inrix_truck_seconds_nonull_10min.zip lmw_2021_inrix_truck_seconds_nonull_10min.csv' with (format csv, header);

\echo 'Creating 2021 AM Peak table at ' `date`
create table npmrds_truck_amp_2021 as 
    select * from npmrds_2021_truck_seconds_nonull_10min
    where
    --Mon-Fri
...
create table npmrds_truck_midd_2021 as
    select * from npmrds_2021_truck_seconds_nonull_10min
    where
    --Mon-Fri
    (extract(dow from measurement_tstamp ) between 1 and 5) and
...
create table npmrds_truck_pmp_2021 as
    select * from npmrds_2021_truck_seconds_nonull_10min
    where
    --Mon-Fri
    (extract(dow from measurement_tstamp ) between 1 and 5) and
...
\echo 'Creating 2021 Overnight table at ' `date`
create table npmrds_truck_ovn_2021 as
    select * from npmrds_2021_truck_seconds_nonull_10min
    where
...
create table npmrds_truck_we_2021 as
    select * from npmrds_2021_truck_seconds_nonull_10min
    where
    --Sat and Sun 6AM-8PM
